...
#devtools::install_github("kindlab/SCIBED")
```
...
A second dataset for H3K9me3 from the same manuscript is included and can be accessed:
```{r}
data(zeller_H3K9me3_matrix)
data(zeller_H3K9me3_metadata)
data(zeller_H3K9me3_peaks)

dim(zeller_H3K9me3_matrix)
```
...
### alternative: a noise vector

If one has a predefined vector of noise-levels per bin, use the `noise_vector` parameter intead of the `noise_level` parameter in `generate_in_silico`.
```{r In silico dataset generation with a noise vector}

# set a chunk of the genome to high noise
noise_vector <- setNames(rep(0, nrow(zeller_H3K4me3_matrix)),rownames(zeller_H3K4me3_matrix))
noise_vector[grepl("chr1:26[0-9][0-9][0-9][0-9][0-9][0-9]", names(noise_vector))] <- 50

# run generate_in_silico()
in_silico_output <- generate_in_silico(x = zeller_H3K4me3_matrix,
                                       grouping_vector = zeller_H3K4me3_metadata,
                                       bin_size = 50000,
                                       target_RD = 10000,
...
gt <- in_silico_output$ground_truth
in_silico_output <- in_silico_output$in_silico_matrix
in_silico_output <- rowSums(as.matrix(in_silico_output[,grepl('Eryths',colnames(in_silico_output))]))

df <- rbind(data.frame(mode = 'groundTruth', bins = names(gt[,"Eryths"]), 
                  value = gt[,"Eryths"]),
            data.frame(mode = 'noise added as vector', bins = names(noise_vector), value = noise_vector),
            data.frame(mode = 'mode = noise vector', bins = names(in_silico_output), value = in_silico_output))

rownames(df) <- NULL
df <- separate(df, bins, c("chromosome",'start','end'), sep = "[\\:\\-]", remove = F,convert = T)

ggplot(df[df$chromosome == 'chr1' & df$start >= 1e7 & df$start <= 4e7,], aes(x = start, y = value)) +
  facet_wrap('mode', ncol = 1, scales = 'free_y') +
  geom_area() +
  theme_bw() +
  labs(x = "chr1:10000000-40000000", y = NULL, title = 'adding bin-specific noise') +
...
```{r run SAVER}
dataPostImputation <- saver(data[["in_silico_matrix"]], 
...
To evaluate **signal enrichment**, we can use the `calculate_enrichment` function or the `calculate_enrichment_bulk` function to measure signal enrichment over a set of pre-defined regions, set by the parameter `regions`. For this tutorial, we'll use the peaks called from the Zeller *et al* dataset as described in the paper (`zeller_H3K4me3_peaks`) and use the pseudobulk-based method (`calculate_enrichment_bulk`). Note that these pre-defined regions should be formatted as a list, with each element of the list being a subset of regions. In this case, each element of the list is the peak calls for a cell type-specific subpopulation of cells.

```{r calculate signal enrichment}
enrichment_preImputation <- calculate_enrichment_bulk(
  x = data[["in_silico_matrix"]], 
  grouping_vector = zeller_H3K4me3_metadata, 
  regions = zeller_H3K4me3_peaks)
enrichment_preImputation$Dataset <- "Pre-Imputation"

enrichment_postImputation <- calculate_enrichment_bulk(
